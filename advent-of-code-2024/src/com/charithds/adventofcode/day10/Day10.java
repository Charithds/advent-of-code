...
class HikeMap {
    ...
    private final List<List<Integer>> startingPositions;

    HikeMap(Integer[][] map, List<List<Integer>> startingPositions) {
        this.map = map;
    ...
    public List<List<Integer>> getStartingPositions() {
        return startingPositions;
    }
}

public class Day10 implements TaskInt {
    /*
    ...
    private void findTrailCount(List<Integer> startingPos, Integer[][] map, int limX, int limY, List<Integer> parent,
                                Set<List<Integer>> nines) {

        Integer x = startingPos.get(0);
        Integer y = startingPos.get(1);
        if (x > limX || y > limY || x < 0 || y < 0) return;
        if (parent != null && map[x][y] - map[parent.get(0)][parent.get(1)] != 1) return;
        if (map[x][y] == -1) return;
        if (map[x][y] == 9) {
...
//        System.out.println("x : " + x + " y: "+ y);
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                ...
                findTrailCount(List.of(x + i, y + j), map, limX, limY, List.of(x, y), nines);
            }
        }
    }
...
//        printMap(input.getMap());
        ...
        for (List<Integer> startingPosition : input.getStartingPositions()) {
            ...
            findTrailCount(startingPosition, input.getMap(), limX, limY, null, nines);
            sum += nines.size();
        }
