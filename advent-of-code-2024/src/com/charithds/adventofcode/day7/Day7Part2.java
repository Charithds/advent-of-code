...
public class Day7Part2 implements TaskInt {
    private final String DAY7_FILE;

    public Day7Part2(String day6File) {
    ...
    public void execute() {
        ...
        for (int k = 0; k < input.size(); k++) {
            ...
            long combinations = (long) Math.pow(3, numbers.size()-1);
            for (long i = 0; i < combinations; i++) {
                String binaryString = String.format("%"+(numbers.size()-1)+"s", Long.toString(i, 3)).replace(" ", "0");
//                System.out.println("Binary string for combination " + i + ": " + binaryString);
                long start = numbers.get(0);
                for (int j = 0; j < binaryString.length(); j++) {
                    if (binaryString.charAt(j) == '1') {
                        start += numbers.get(j + 1);
                    } else if (binaryString.charAt(j) == '2') {
                        start *= numbers.get(j + 1);
                    } else {
                        start = Long.parseLong(start + "" + numbers.get(j + 1));
                    }
                }
                if (start == key) {
                    total += key;
